import operator, os, warnings, time, re, yaml, datetime, gc, pickle
import pandas as pd
...
class Dicom:
    ...
    def get_contours(self, path, exceptions, select):
        ...
        if self.dataset == "radcure":
            ...
            # ROILIST in neck levels...
            # rtstruct = ['LEVEL IA', 'LEVEL IB_RT', 'LEVEL IB_LT', 'LEVEL II_RT', 'LEVEL II_LT', 'LEVEL III_RT',
            # 'LEVEL III_LT', 'LEVEL IV_RT', 'LEVEL IV_LT', 'LEVEL V_RT', 'LEVEL V_LT', 'RETROPHARYNGEAL_RT', 'RETROPHARYNGEAL_LT']
            assert self.folder is not None
            rtstruct = pdcm.read_file(f'/cluster/projects/radiomics/RADCURE-Neck-level-contours/RDACURE-Neck_level-contours-bulky-nodes/{self.folder}_RTst_Neck_level.dcm')
        ...
        if self.dataset == "radcure":
            ...
            # should be the same for neck levels...
            bad = ['ALL LEVELS', 'ROI-14', 'ROI-13']
            self.roi_select = self.roi_list.copy()
            self.roi_select.sort()
            self.roi_select = [r for r in self.roi_select if r not in bad ]
